...
SquareMap::SquareMap(glm::vec2 pos, glm::vec2 size, Texture sprite, glm::vec3 color)
    : Position(pos), Size(size), Color(color), Rotation(0.0f), Sprite(sprite) { }

void SquareMap::Draw(Render& renderer)
{
    ...
    for (unsigned int row = 0; row < 10; row++) {
        for (unsigned int col = 0; col < 20; col++) {
            ...
            glm::vec2 Position_written = glm::vec2(position_inc_x, position_inc_y);
    ...
    position_inc_x = 0;
    position_inc_y = 0;
    Texture fillerSprite = ResourceManager::GetTexture("block");
    for (unsigned int row = 0; row < 10; row++) {
        for (unsigned int col = 0; col < 20; col++) {
            position_inc_x = row * 40.0f;
            position_inc_y = col * 40.0f;
